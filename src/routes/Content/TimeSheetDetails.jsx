    ...
    <div className='categoies-container d-flex flex-wrap  justify-content-around mt-3'>
        {categ.map((obj,key)=>(
            <div onClick={()=>handleRowClick(obj.category_id,obj.category_name)} key={key} style={{borderRadius:"15px",backgroundColor:"white"}} className="d-flex flex-column m-2 justify-content-between align-items-center shadow-sm p-2">
                ...
                <ProgressBar variant={(obj.payed_orders/obj.order_counter) > 0.70 ? "success" : (obj.payed_orders/obj.order_counter) > 0.35 ? "warning" : "danger"} animated ={(obj.payed_orders/obj.order_counter)!=1} style={{width:"100%"}} now={(obj.payed_orders/obj.order_counter)*100} label={`${(obj.payed_orders/obj.order_counter)*100}%`}  />
                :
